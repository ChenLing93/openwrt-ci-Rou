...
## 项目说明
- 默认管理地址：**`192.168.5.1`** 默认用户：**`root`** 默认密码：**`none`**
...
- `ImmortalWrt` 和 `LibWrt` 分别通过 rebase 和 merge 进行更新，相互印证，从laipeng668大佬仓库复制而来，自用固件，稳定性未知。
...
- 编译大概需要 1-2 小时，编译完成后在仓库主页 [Releases](https://github.com/ChenLing93/openwrt-ci-Rou/releases) 对应 Tag 标签内下载固件。
